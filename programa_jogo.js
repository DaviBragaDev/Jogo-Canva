...
//predefinição do desenho do mapa
...
//PREDEFINIÇÃO DE DESENHO DO NPC
function NPC(){
...
//PREDEFINIÇÃO DO DESENHO DO PERSONAGEM
...
// FUNÇÃO QUE ATUALIZA  O PERSONAGEM E TRABALHA A COLISAO DO MAPA 
function atualizaPersonagem(tiles_horizontal, step, posicao_personagem){
    ...
    //BLOQUEIA A MOVIMENTAÇÃO CASO CHEGUE EM UM BLOCO DE COLISAO 
    if (colisao[prox_linha*tiles_horizontal+prox_coluna]){
        return;
    }
    
    //DIREÇÃO PARA ONDE O BONECO OLHA 
    if (direcao == 3) { //w
        cameraY -= step;
    }
    if (direcao == 1) { //a
